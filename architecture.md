...
- AD21 Optimistic-Locking Enforcement
    ...
    - **Decision:** Hybrid two-layer enforcement.
        ...
        4. The scheduler's `open_assignments` and `close_completed_events` functions are refactored to load event IDs only in the initial query, then process each event individually with one retry on `StaleDataError` (log WARNING on retry, ERROR if skipped).
    ...
    - **Alternatives rejected:** Pessimistic row locking for general edits — too blunt; blocks readers and adds infrastructure complexity unjustified at expected edit-rate. JSON 409 responses — not applicable; all in-scope routes return HTML.
