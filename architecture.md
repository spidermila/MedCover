...
- AD10 Background Task / Scheduler Architecture
    ...
    - Notes
        - Scheduler tasks implemented: (1) drain the email outbox (`process_email_queue`) — runs every `MAIL_QUEUE_INTERVAL_SECONDS` (default 3 s); (2) auto-open assignments at `assignments_open_at` datetime; (3) send unfilled-spot reminder emails per `reminder_schedule`; (4) auto-complete Events after `end_datetime`; (5) send admin digest emails; (6) run scheduled DB backups; (7) delete expired work-report xlsx files from `instance/work_report/` (runs hourly).
        - The scheduler's main loop wakes every 5 seconds to run any due jobs (each task itself is scheduled at its own interval — see above); no message broker needed at this volume.
...
- AD14 CSRF Protection and Input Validation
    ...
    - **Decision:**
        ...
        4. **XSS:** Jinja2 auto-escape is always on. `{{ var | safe }}` is prohibited for user-supplied content. A `Content-Security-Policy` header is added via a `@app.after_request` hook in non-dev environments. The policy is: `default-src 'self'; script-src 'self' https://cdn.jsdelivr.net 'nonce-<PER_REQUEST>'; style-src 'self' https://cdn.jsdelivr.net 'nonce-<PER_REQUEST>'; font-src 'self' https://cdn.jsdelivr.net data:; img-src 'self' data:; connect-src 'self' https://cdn.jsdelivr.net`. Both `script-src` and `style-src` use a per-request nonce (16 random hex bytes, regenerated in `before_request` and exposed as `g.csp_nonce`); neither directive uses `'unsafe-inline'`. All inline `<script>` / `<style>` blocks in templates carry `nonce="{{ g.csp_nonce }}"`. Third-party libraries that inject `<style>` at runtime (FullCalendar, Flatpickr) are handled two ways: FullCalendar reads the nonce from `<meta name="csp-nonce">` in `base.html`; libraries that do not consult the meta tag are covered by a small `document.createElement` shim in `base.html` that auto-nonces every created `<style>` element. All JS is in external files — no inline `onclick`/`onchange` attributes. The `https://` scheme is explicit so the policy works correctly when the app is served over HTTPS.
...
- AD15 Outbound Email Delivery Strategy
    ...
    - **Decision:** **DB-backed outbox queue** (`outbox_email` table) with two coexisting drain paths.
        ...
        - **Failure semantics**: SMTP failure on the *legacy* path increments `retry_count`; the row stays `pending` until `retry_count >= MAX_RETRIES` (3), at which point `status` is set to `failed`. SMTP failure on the *batched* path leaves every included row `pending` with `retry_count += 1`; on `MAX_RETRIES` exhaustion all rows in the batch are marked `failed` and a single audit-log entry summarises the batch. If the referenced event is deleted mid-flight (`event_id IS NULL` after the FK's `ON DELETE SET NULL`), that row is unrecoverable and is deleted outright (`db.session.delete()`) rather than marked `failed` — it carries no `last_error` and the batch continues with the remaining rows.
...
- AD21 Optimistic-Locking Enforcement
    - **Status:** Decided
    - **Context:** The application has had a `version` integer column on nine models (UserAccount, EventTemplate, Event, EventSpot, MasterEvent, EquipmentType, EquipmentItem, DigestSchedule, DigestBlock) since the initial design. Edit routes increment `entity.version += 1` before committing, and several of them call `check_version_conflict()` to compare the submitted form version with the current DB value. However, the ORM's `version_id_col` mapper argument was never declared, so SQLAlchemy never added a `WHERE version = <old>` clause to UPDATE statements. This meant a concurrent edit between the session SELECT and the UPDATE was silently accepted — the column incremented but the conflict was never caught at the DB layer. Routes that do not call `check_version_conflict` (about half) have no protection at all.
    - **Options**
        - **Pure app-level `check_version_conflict`** — already partially in place; requires every future route author to remember to call it; does not help paths the check is missing from; still has a small TOCTOU window between the check and the commit.
        - **Pure ORM `version_id_col`** — SQLAlchemy adds `AND version = <old>` to every UPDATE automatically; implicit and complete. Downside: `StaleDataError` surfaces deep inside the ORM stack; without app-layer handling the user gets a 500. Routes that already show a user-friendly flash message lose that UX without extra effort.
        - **Hybrid: keep app-level check for HTML POST UX + add ORM enforcement as safety net** — `check_version_conflict` continues to provide an early, user-friendly redirect (flash + re-render) for the common case; `version_id_col` catches any path the app-level check misses and any concurrent commit that slips through the TOCTOU window. Scheduler paths (no HTML form) handle `StaleDataError` directly with retry logic.
    - **Decision:** Hybrid two-layer enforcement.
        1. `__mapper_args__ = {"version_id_col": version, "version_id_generator": False}` is declared on all nine models. `version_id_generator: False` means the application increments `version` itself; SQLAlchemy only adds the `WHERE version = <old>` guard. Callers must bump `entity.version += 1` before commit.
        2. `app/utils.py` gains two helpers: `bind_form_version(entity, form_value)` — uses `sqlalchemy.orm.attributes.set_committed_value` to set the ORM's committed-value snapshot to the form-submitted integer without marking the column dirty; and `commit_or_stale(redirect_url)` — wraps `db.session.commit()` and catches `StaleDataError`, rolling back and redirecting with a flash message.
        3. All HTML-form POST routes that edit versioned entities call `check_version_conflict` first, then `bind_form_version`, then `commit_or_stale`. Pure-button POSTs (activate/deactivate/archive/unarchive/batch) load the entity fresh and do not need the form-version check.
        4. The scheduler's `open_assignments` and `close_completed_events` functions are refactored to load event IDs only in the initial query, then process each event individually with one retry on `StaleDataError` (log WARNING on retry, ERROR if skipped). `run_send_reminders` rolls back an event whose commit hits `StaleDataError` (dropping its enqueued emails) and leaves it for the next run.
        5. An app-wide `StaleDataError` error handler backs up every write that does not go through `commit_or_stale` (button POSTs, table-manager XHR, status transitions, spot edits, background-ish writes on GET). It rolls back and either returns `409` with `{"ok": false, "error": …}` to `csrfFetch()` callers (detected by the `X-CSRFToken` header) or flashes the conflict message and redirects to the same-origin referrer (dashboard otherwise).
    - **Consequences:** Every edit commit now causes `StaleDataError` when the DB row was modified by a concurrent actor since the session loaded the object. Routes without `bind_form_version` must still bump `entity.version += 1`; missing the bump causes the version to stay at the committed value in-memory, and SQLAlchemy will SET the version to the same number it was, which passes the WHERE check but does not increment the counter (silent bug). Reviewers must verify new edit routes follow the full pattern.
    - **Alternatives rejected:** Pessimistic row locking for general edits — too blunt; blocks readers and adds infrastructure complexity unjustified at expected edit-rate. Per-route JSON 409 handling — the table-manager and other XHR endpoints get their 409 from the app-wide handler instead.
    - **Cross-references:** AD12 — pessimistic UPDLOCK on spot assignment is unchanged and unaffected by this decision.
...
| **OutboxEmail** | to_email, subject, body, html_body, notification_type, instance_name, user_id, event_id, change_type, change_value, send_after, status (pending/sent/failed/skipped), retry_count, last_error, created_at, sent_at | All outbound emails are queued here (table `outbox_email`) and drained by the scheduler (see AD15). Event-related notifications additionally carry `user_id`/`event_id`/`send_after` for deferred, proximity-tiered batching. Provides delivery audit trail. |
| **DigestSchedule** | enabled flag, frequency_hours, preferred_hour, last_sent_at, email_subject, header/footer HTML blocks | Configures who receives the admin digest and how often; managed via Admin Digest UI |
