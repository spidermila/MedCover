...
- With `AUTH_MODE=oidc` and `LDAP_URI` set, MedCover copies its users from the MemberBase directory: the scheduler every 15 minutes and each login for the person logging in. Holding a MedCover role in MemberBase is what gives a person access to MedCover. An invited person („Pozvaný“) is activated by their first MedCover login; they need not open MemberBase first. Name, email, phone, status, Místní skupina, kind (member or external), MedCover roles and qualifications (definitions, hierarchy and holders) follow the directory. People whose access is revoked or who are archived in MemberBase are archived in MedCover: their sessions end and pending emails are dropped, but their name stays on the events they took part in; access granted again brings them back. Changes are recorded in the audit log. While the sync is on, what the directory owns is read-only in MedCover: no role has the user and qualification editing permissions (user page, status and batch actions, „Kvalifikace“ create/edit/delete), the profile keeps name and phone read-only (preferences stay editable), and the Google Sheets event import is off because it creates users. After changes that affect who may be responsible, each open event's „Zodpovědná osoba“ is re-checked. New settings `LDAP_URI`, `LDAP_CA_CERT`, `LDAP_BASE_DN`, `LDAP_SYNC_PASSWORD`; the image now builds `python-ldap`. (#560)
...
- Debriefing management view (/debriefing/manage) is now paginated at 20 events per page, with a pager that preserves the active „Od data“ / „Do data“ range; a page number past the last page redirects to the last page. The record counter above the list now reports the total number of matching events instead of the number shown on the current page. (#410)
...
- Scheduled DB backup trigger is now configurable to the minute (previously hour-only). The „Hodina spuštění“ field is replaced by a „Čas spuštění“ time picker (HH:MM), interpreted in the app's configured timezone — the UI previously mis-labelled the field as UTC even though the value was already local. A migration adds `backup_schedule_minute` (default 0, so existing schedules keep the same trigger time). (#497)
- Ad-hoc „Spustit zálohu nyní“ backups no longer suppress the scheduled daily backup: the scheduled run fires at its configured time regardless of any ad-hoc backups already created that day, and remains capped at one *scheduled* run per local day. Extra ad-hoc backups still count against „Počet uchovávaných záloh“ and may cause older automatic backups to be pruned sooner. (#497)
- Backup filenames now carry an explicit `_UTC` suffix (e.g. `medcover_backup_20260904_143022_123456_UTC.zip`) to remove any ambiguity about which timezone the embedded timestamp is in. Pre-existing filenames without the suffix stay downloadable and restorable. (#497)
