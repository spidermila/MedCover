...
| `LDAP_URI` | The MemberBase directory (LDAPS). With `AUTH_MODE=oidc`, setting it turns on the user sync: the scheduler copies everyone with a MedCover role every 15 minutes (and once at start), and each login copies the person first. Empty: no sync | `ldaps://openldap:1636` |
| `LDAP_CA_CERT` | CA certificate of the directory's TLS certificate; empty uses the system store. In the dev stack, `web` and `scheduler` mount the directory's `ldap_certs` volume at `/certs` | `/certs/ca.crt` |
| `LDAP_BASE_DN` | Directory base DN; the sync binds as `cn=medcover-sync,ou=services,<base DN>` | `dc=example,dc=org` |
| `LDAP_SYNC_PASSWORD` | Password of `cn=medcover-sync` (the directory's `LDAP_MEDCOVER_SYNC_PASSWORD`) | — |
...
The `AUTH_MODE`, Keycloak and `LDAP_*` variables apply to both the `web` and the `scheduler` container: both build the app, and `AUTH_MODE=oidc` without `OIDC_CLIENT_SECRET` or `KEYCLOAK_INTERNAL_URL` stops them at startup. The sync overwrites name, email, phone, status, Místní skupina, roles and qualifications with the directory's values and never deletes anyone: people it no longer sees (no MedCover role any more, or archived in MemberBase) are archived, so their name stays on past events. When the directory returns nobody at all, no MedCover roles, or holdings without qualifications, it changes nothing; a single person or qualification that cannot be written is logged and skipped. While the sync is on, no role has the permissions to edit those fields or the qualification definitions (user page, batch actions, „Kvalifikace“, name and phone on the profile), and the event import is off.
