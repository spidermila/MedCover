    ...
    def test_no_audit_entry_when_nothing_changes(self, app: object, admin_client: object) -> None:
        ...
        with app.app_context():
            count = db.session.scalar(
                db.select(db.func.count(AuditLogEntry.id)).where(
                    AuditLogEntry.entity_type == "UserAccount", AuditLogEntry.entity_id == uid
                )
            )
            assert count == 0

    def test_only_qualification_audit_when_only_qualifications_change(self, app: object, admin_client: object) -> None:
...
class TestInvites:
    ...
    def test_invite_blocked_for_existing_user(self, app: object, admin_client: object) -> None:
        ...
        with app.app_context():
            inv = db.session.scalar(
                db.select(RegistrationInvite).where(RegistrationInvite.email == "existing@example.com")
            )
            assert inv is None
    ...
    def test_create_invite_with_custom_subject_and_message(self, app: object, admin_client: object) -> None:
        ...
        with app.app_context():
            ...
            inv = db.session.scalar(
                db.select(RegistrationInvite).where(RegistrationInvite.email == "custom@example.com")
            )
            assert inv is not None
    ...
    def test_cancel_invite(self, app: object, admin_client: object) -> None:
        ...
        with app.app_context():
            inv = db.session.scalar(
                db.select(RegistrationInvite).where(RegistrationInvite.email == "cancel@example.com")
            )
            assert inv is not None
            inv_id = inv.id
    ...
    def test_cancel_invite_writes_audit_log(self, app: object, admin_client: object) -> None:
        ...
        with app.app_context():
            inv = db.session.scalar(
                db.select(RegistrationInvite).where(RegistrationInvite.email == "cancelaudit@example.com")
            )
            inv_id = inv.id
    ...
    def test_cancelled_invite_allows_new_invite(self, app: object, admin_client: object) -> None:
        ...
        with app.app_context():
            inv = db.session.scalar(
                db.select(RegistrationInvite).where(RegistrationInvite.email == "retry@example.com")
            )
            inv_id = inv.id
        admin_client.post(f"/users/invites/{inv_id}/cancel", follow_redirects=True)
    ...
    def test_create_invite_queues_outbox_email(self, app: object, admin_client: object) -> None:
        ...
        with app.app_context():
            ...
            inv = db.session.scalar(
                db.select(RegistrationInvite).where(RegistrationInvite.email == "outbox@example.com")
            )
            assert inv is not None
    ...
    def test_resend_invite_creates_new_outbox_entry(self, app: object, admin_client: object) -> None:
        ...
        with app.app_context():
            inv = db.session.scalar(
                db.select(RegistrationInvite).where(RegistrationInvite.email == "resend@example.com")
            )
            assert inv is not None
    ...
    def test_new_user_gets_member_role(self, app: object, client: object) -> None:
        ...
        with app.app_context():
            ...
            assert Role.MEMBER in role_names
...
class TestBatchAction:
    ...
    def _make_users(self, app, count: int = 3):
        ...
        with app.app_context():
            ...
            for i in range(count):
                ...
                db.session.add(u)
            db.session.commit()
            users = db.session.scalars(db.select(UserAccount).where(UserAccount.email.like("batchuser%@test.cz"))).all()
    ...
    def test_batch_writes_audit_log(self, app, admin_client):
        ...
        with app.app_context():
            entries = db.session.scalars(db.select(AuditLogEntry).where(AuditLogEntry.action_type == "edit")).all()
    ...
    def test_batch_requires_permission(self, member_client):
        ...
        resp = member_client.post(
            "/users/batch",
            data={"user_ids": ["00000000-0000-0000-0000-000000000001"], "action": "add_role", "role_id": "1"},
        )
        assert resp.status_code == 403
